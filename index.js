...
function fixTypo(myArray, index, myString) {
  myArray[index].name = myString;
  return myArray[index];
}
console.log(fixTypo(artists, 8, "Vincent Van Gogh"));
console.log(artists[8]); // Checked and corrected
...
function get20s(myArray){
  ...
  for (let i = 0; i < myArray.length; i++) {
    ...
    if (yearHolderArray[0] >= 1900 && yearHolderArray[2] < 2000) {
      artistHolderArray.push(myArray[i].name);
    }
  }
...
*/
const testArray = [{name: "Sam", city: "Madison"}, {name: "Bob", city: "Austin"}, {name: "Sara", city: "Oconomowoc"}];

function removeArtist(myArray, index) {
    myArray.splice(index, 1);
    return myArray.length;
  }
  console.log(removeArtist(testArray, 0));
...
function addArtist(myArray, myObject){

    myArray.push(myObject);
    return myArray;
  }

const victoriaArtist = {
  id: 20, 
  name: "Victoria", 
  years: "1993 - 2020",
  genre: "Web Design",
  nationality: "USA",
  bio: "Working and drinking coffee in Nashville, TN."
}
console.log(addArtist(artists, victoriaArtist));
...
function lotsOfArt(myArray){
  ...
  for (let i = 0; i < myArray.length; i++) {
    if (myArray[i].paintings > 100) {
      holderArray.push(myArray[i].name);
    }
  }
  return holderArray;
}
...
<div id="artist">
  <div class="image"> 
  <img src="https://images.fineartamerica.com/images/artworkimages/mediumlarge/3/starry-night-by-vincent-van-gogh-vincent-van-gogh.jpg"/></div><div class = "name">
...
const newArtist = {
  name: "Sterling Mount", 
  nameLink: "https://en.wikipedia.org/wiki/Vincent_van_Gogh",
  image: "https://images.fineartamerica.com/images/artworkimages/mediumlarge/3/starry-night-by-vincent-van-gogh-vincent-van-gogh.jpg",
  bio: "Very talented woofer."
}
...
function randomize(myArray){
    ...
    return myArray.sort(() => (Math.random()-.5));
// Going through elements of the array, they're being sorted up or down based on a (roughly) random probability of a +/- number
  }

  console.log(randomize(colorArray));
  console.log(randomize(colorArray));

 /* STRETCH 3: Use advanced array methods (.map, .reduce, .filer) to refactor your MVP code (create an array of all artists born in the 1900s with .filter, for example) */

// funtion getCenturyFilter(myArray) {
//   // let artistHolderArray = [];
//   // let yearHolderArray = [];
//   // for (let i = 0; i < myArray.length-1; i++) {
//   //   if (myArray.years)
//   // }
//   return myArray.years >= 1900;
// }
// let artistHolderArray = artists.filter(getCenturyFilter);


// // console.log(getCenturyFilter(artists));
